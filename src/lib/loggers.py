...
class ImageLogger(Callback):
    ...
    def on_train_batch_end(self, trnr, pl_module, outputs, batch, batch_idx, **kwargs):
        if not self.disabled and (pl_module.global_step > 0 or self.log_first_step):
            self.log_img(pl_module, batch, batch_idx, split="train")

    def on_validation_batch_end(self, trnr, pl_module, outputs, batch, batch_idx,
                                dataloader_idx):
        ...
        if hasattr(pl_module, 'calibrate_grad_norm'):
            if (
                ...
                self.log_gradients(trnr, pl_module, batch_idx=batch_idx)
